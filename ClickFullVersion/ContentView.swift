    ...
    var body: some View {
        ...
        NavigationSplitView {
            NavigationLink(destination: Game()) {
                Text("Play")
            }.navigationTitle("Click").navigationBarHidden(false)
            Text("\n")
            ...
            Text("").frame(minWidth: 0, idealWidth: .infinity, maxWidth: .infinity, minHeight: 0, idealHeight: .infinity, maxHeight: .infinity, alignment: .center)
            if (hacks == 1){
                Button(action: {
                    hackMenu = true
                }, label: {
                    Text(" ")
                }).alert(Text("Hack Menu"), isPresented: $hackMenu, actions: {
                    Button(action: {
                        if (hideHSDWarning) {
            ...
            Button(action: {
                ...
                Text("Reset")
            }).alert(Text("Are you sure you want to reset?"), isPresented: $reset, actions: {
            ...
            if (hacks == 1 && !hideHSDWarning) {
                Text("Hacked Save Data").alert("Tap above the reset button to access the hack menu.", isPresented: $hackMenuMessage, actions: { })
            }
        } detail: {
